...
(function() {
  var Bluebird, ControllerManager, FrontEnd, FuncDetails, _, frontEndJS;
  ...
  module.exports = FrontEnd = (function() {
    ...
    FrontEnd.prototype.registerRoutes = function(app, dirname, dynoScripts) {
      var initPromise;
      initPromise = dirname ? this.controllerManager.readdir(dirname) : Bluebird.resolve(true);
      return initPromise.then((function(_this) {
        return function() {
          ...
          return app.get('/get-batch', function(req, res) {
            var response;
            response = {};
            return Bluebird.all(_.map(req.query, function(value, key) {
      ...
      if (_.isFunction(object)) {
        ...
        output += tabs2 + ((function() {
          ...
          for (i = 0, len = object.length; i < len; i++) {
            object_item = object[i];
            results.push(this.convertToFrontEndRaw(object_item, depth + 1));
          }
          return results;
        }).call(this)).join(",\n" + tabs2) + "\n";
        output += tabs + "]";
        ...
        output += tabs2 + ((function() {
          ...
          for (field in object) {
            value = object[field];
            results.push(field + ": " + this.convertToFrontEndRaw(value, depth + 1));
          }
          return results;
        }).call(this)).join(",\n" + tabs2) + "\n";
        output += tabs + "}";
