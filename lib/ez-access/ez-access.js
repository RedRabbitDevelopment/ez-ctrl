  ...
  (function(generator) {
    ...
    return EZAccess = {
      ...
      createBatch: function() {
        var batchProcessor, key, val;
        batchProcessor = _.extend({}, EZAccess, {
          _makeRequest: function(routeDetails, args, controllerName, methodName) {
            return this.requests.push({
              controllerName: controllerName,
              methodName: methodName,
              args: this._extractData(routeDetails.argList, args)
            });
          },
          finish: function() {
            return this._makeRequestBase('GET', '/batch-get' + this._constructQuery({
              requests: this.requests
            }));
          }
        });
        for (key in batchProcessor) {
          val = batchProcessor[key];
          if (val._makeRequest) {
            val._makeRequest = batchProcessor._makeRequest;
          }
        }
